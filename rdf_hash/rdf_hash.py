...
class EhisCanon:
    ...
    **Note:** there has been further development of this logic since the original
    ...
    def get_hash(self, gr, hash_fn=sha256):
        """
        ...
        :return: Graph digest.
        """
        return hash_fn(self.serialize(gr)).digest()


    def serialize(self, gr):
        """
        ...
        subj_strings = []
        res = bytearray()
        self._orig_subj = None

        for subject in set(gr.subjects()):
            visited_nodes = set()
            self._orig_subj = subject
            #import pdb; pdb.set_trace()
            subj_strings.append(
                    self._encode_subject(subject, visited_nodes, gr))
    ...
    def _encode_subject(self, subject, visited_nodes, gr):
        """
        ...
        if isinstance(subject, BNode):
            if subject in visited_nodes:
                return self.ORIG_S if subject == self._orig_subj else b''
            else:
                visited_nodes.add(subject)
                res = bytearray(self.BNODE)
        else:
            res = bytearray(subject.n3().encode('utf-8'))
        res += self._encode_props(subject, visited_nodes, gr)

        return res


    def _encode_props(self, subject, visited_nodes, gr):
        """
        ...
        for pred in preds:
            ...
            objs = sorted([
                    self._encode_object(obj, visited_nodes, gr)
    ...
    def _encode_object(self, obj, visited_nodes, gr):
        """
        ...
        if isinstance(obj, BNode):
            return self._encode_subject(obj, visited_nodes, gr)
        else:
